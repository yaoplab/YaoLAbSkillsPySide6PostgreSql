"""ParentsList — statut de paiement de tous les parents."""
...
class ParentsList(QScrollArea):
    ...
    def _setup_ui(self):
        ...
        for lbl, w in [("Parent", 200), ("Eleves", 80), ("Mode", 90), ("Du", 100),
                        ("Encaisses", 100), ("Solde", 100), ("Progression", 150)]:
            l = QLabel(lbl)
            l.setFixedWidth(w)
    ...
    def _load(self):
        ...
        # Par parent : total des frais de ses enfants - paiements recus
        cur.execute("""
            SELECT par.id, par.first_name, par.last_name,
                   COALESCE(SUM(CASE WHEN prog.id IN (13,23) THEN %s ELSE %s END), 0) as total_du,
                   COALESCE(SUM(pay.paid_amount), 0) as total_paid,
                   COUNT(DISTINCT stu.aecuser_ptr_id) as nb_enfants,
                   MAX(COALESCE(sch.payment_mode, 'inconnu')) as mode
            ...
            ORDER BY (COALESCE(SUM(CASE WHEN prog.id IN (13,23) THEN %s ELSE %s END), 0) - COALESCE(SUM(pay.paid_amount), 0)) DESC
        """, (LYCEE, COLLEGE, LYCEE, COLLEGE))

        for row in cur.fetchall():
            pid, fn, ln, total_du, total_paid, nb_enfants, mode = row
            ...
            # Nb enfants
            kids = QLabel(str(nb_enfants))
            kids.setFixedWidth(80)
            kids.setStyleSheet(f"font-size: {theme_manager.font_size(11)}px; color: {p.text_soft};")
            rl.addWidget(kids)

            # Mode
            md = QLabel(mode or "—")
            md.setFixedWidth(90)
            md.setStyleSheet(f"font-size: {theme_manager.font_size(11)}px; color: {p.text_soft};")
            rl.addWidget(md)

            # Total du
            du_l = QLabel(_fmt(total_du))
            du_l.setFixedWidth(100)
            du_l.setStyleSheet(f"font-size: {theme_manager.font_size(12)}px; color: {p.text_strong};")
            rl.addWidget(du_l)

            # Paye
            paid_l = QLabel(_fmt(total_paid))
