...
class _SlotBar(QFrame):
    ...
    def _STYLE_ACTIF(self):
        ...
        return f"""
            ...
            border-radius: {ds.space_xxs}px; padding: {ds.space_xxs//2}px;
        """
    ...
    def _STYLE_NEXT(self):
        p = theme_manager.palette
        return f"""
            background: {p.surface_variant}; border: 1px dashed {p.outline_variant};
            border-radius: {ds.space_xxs}px; padding: {ds.space_xxs//2}px;
        """
    ...
    def _build_ui(self):
        ...
        self._code.setStyleSheet(
            f"font-weight: bold; font-size: {theme_manager.font_size(10)}px; border: none; padding: 0;"
        )
        layout.addWidget(self._code)

        self._label = QLabel('')
        self._label.setStyleSheet(
            f"font-size: {theme_manager.font_size(10)}px; color: {theme_manager.palette.text_strong}; border: none; padding: 0;"
        )
        ...
        for letter in ['A', 'B', 'C', 'D']:
            cb = QCheckBox(letter)
            cb.setEnabled(False)
            cb.setStyleSheet(f"font-size: {theme_manager.font_size(9)}px; border: none; padding: 0;")
    ...
    def restyle(self):
        """Ré-applique le style selon l'état avec la palette courante (changement de thème)."""
...
class EvalManagerWindow(QDialog):
    ...
    def __init__(self, eval_type: str, termsubject_id: int,
        ...
        self.setMinimumSize(ds.window_width * 3 // 4, ds.window_height * 3 // 4)  # 900×600
        self.setModal(False)
    ...
    def _build_ui(self):
        ...
        splitter.setSizes([ds.jugements_width + ds.sidebar_width + ds.space_md, ds.kpi_card_height * 6 + ds.space_md])  # 397, 500
        ...
        self._status_lbl.setStyleSheet(f"font-size: {theme_manager.font_size(9)}px; color: {ds.p.text_strong}; padding: {ds.space_xxs//2}px 0;")
    ...
    def _restyle(self):
        ...
        try:
            self._left_panel.setStyleSheet(panel_qss)
            self._right_panel.setStyleSheet(panel_qss)
            self._status_lbl.setStyleSheet(
                f"font-size: {theme_manager.font_size(9)}px; color: {ds.p.text_strong}; padding: {ds.space_xxs//2}px 0;"
            )
            if hasattr(self, '_legend') and self._legend:
                self._legend.setStyleSheet(
                    f"font-size: {theme_manager.font_size(7)}px; color: {p.text_strong}; border: none; padding: 0;"
                )
            # Bouton sauvegarde
            if hasattr(self, '_save_btn') and self._save_btn:
                self._save_btn.setStyleSheet(f"""
                    QPushButton {{ background: {p.success}; color: white; font-weight: bold;
                                  padding: {ds.space_xs}px {ds.space_md}px; border-radius: {d.radius}px; font-size: {theme_manager.font_size(13)}px; }}
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._legend = QLabel('')
        self._legend.setStyleSheet(
            f"font-size: {theme_manager.font_size(7)}px; color: {theme_manager.palette.text_strong}; border: none; padding: 0;"
        )
        ...
        self._list_layout.setSpacing(ds.space_xxs)  # 4px (3 INTERDIT)
    ...
    def _build_tabs(self, layout: QVBoxLayout):
        ...
        for i in range(12):
            ...
            btn.setStyleSheet(f"""
                QPushButton {{ font-size: {theme_manager.font_size(8)}px; font-weight: bold; padding: {ds.space_xxs//4}px {ds.space_xxs}px;
                              background: {p.outline_variant}; color: {p.text_disabled}; border-radius: {d.radius}px; }}
                QPushButton:checked {{ background: {theme_manager.palette.success}; color: white; }}
                QPushButton:hover {{ background: {p.outline_variant}; }}
                QPushButton:checked:hover {{ background: {theme_manager.palette.success}; }}
            """)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._save_btn = QPushButton("Enregistrer cette évaluation")
        self._save_btn.setStyleSheet(f"""
            QPushButton {{ background: {theme_manager.palette.success}; color: white; font-weight: bold;
                          padding: {ds.space_xs}px {ds.space_md}px; border-radius: {d.radius}px; font-size: {theme_manager.font_size(13)}px; }}
            QPushButton:hover {{ background: {theme_manager.palette.success}; }}
        """)
    ...
    def _update_tabs(self):
        ...
        for i, bar in enumerate(self._bars):
            ...
            if active:
                btn.setStyleSheet(f"""
                QPushButton {{ font-size: {theme_manager.font_size(8)}px; font-weight: bold;
                              padding: {ds.space_xxs // 4}px {ds.space_xxs}px;
                              background: {p.success}; color: white; border-radius: {ds.radius_xs}px; }}
                    QPushButton:hover {{ background: {p.success}; }}
                """)
            else:
                btn.setStyleSheet(f"""
