...
class FilterSection(QFrame):
    ...
    def _STYLE(self):
        ...
        return f"""
            QFrame#section {{
                ...
                border-radius: {ds.space_xxs}px;
            }}
            QPushButton {{
                font-size: {theme_manager.font_size(8)}px;
                padding: {ds.space_xxs//4}px {ds.space_xxs}px;
                border: 1px solid {p.outline_variant};
                border-radius: {ds.space_xxs//2}px;
                background: {p.surface_variant};
                color: {p.text_strong};
                min-height: {ds.font_label_lg}px;  # 13px
            }}
            QPushButton:hover {{ background: {p.outline_variant}; }}
            QCheckBox {{ font-size: {theme_manager.font_size(9)}px; spacing: {ds.space_xxs//2}px; }}
            QCheckBox::indicator {{
            ...
            QCheckBox:disabled::indicator {{ background: {p.outline_variant}; border-color: {p.outline_variant}; }}
        """
    ...
    def __init__(self, title: str, hide_inactive: bool = False, parent=None):
        ...
        root.setContentsMargins(ds.space_xxs, ds.space_xxs, ds.space_xxs, ds.space_xxs)
        root.setSpacing(ds.space_xxs)

        header = QHBoxLayout()
        header.setSpacing(ds.space_xxs)
        lbl = QLabel(title)
        lbl.setFont(QFont('Segoe UI', 8, QFont.Bold))
        lbl.setStyleSheet(f'color: {theme_manager.palette.text_strong}; border: none;')
        ...
        self._checkboxes_row.setSpacing(ds.space_xxs)
    ...
    def set_slot_active(self, key: str, active: bool) -> None:
        ...
        if active:
            ...
            cb.setStyleSheet(f'color: {theme_manager.palette.primary}; font-weight: bold;')
            cb.setVisible(True)
        else:
            self._active_slots.discard(key)
            cb.setStyleSheet(f'color: {theme_manager.palette.text_disabled};')
    ...
    @safe_slot("Unknown._on_checkbox_changed")
    ...
    @safe_slot("Unknown._on_toggle_all")
    ...
    @safe_slot("Unknown._on_toggle_comments")
